...
export function GeneratedLandingView({ data }: { data: LandingData }) {
  ...
  return (
    <main className={`relative min-h-screen overflow-hidden bg-gradient-to-b ${meta.bg} text-white`}>
      <style>{`
        @keyframes laqtaFloat { 0%,100% { transform: translateY(0) rotate(-1deg); } 50% { transform: translateY(-14px) rotate(1deg); } }
        @keyframes laqtaUp { from { opacity:0; transform: translateY(22px); } to { opacity:1; transform: translateY(0); } }
        @keyframes laqtaPulse { 0%,100% { opacity:.25; transform: scale(1); } 50% { opacity:.7; transform: scale(1.08); } }
        .laqta-float { animation: laqtaFloat 7s ease-in-out infinite; }
      ...
      <section className="relative z-10 mx-auto grid max-w-6xl items-center gap-10 px-5 pb-12 pt-4 sm:px-8 lg:grid-cols-[1fr_1.15fr]">
        <div className="laqta-up space-y-7 text-center lg:text-right">
          ...
          <div className="flex flex-wrap justify-center gap-3 lg:justify-start">
            <a href={ctaHref(data)} target="_blank" rel="noreferrer" className="inline-flex min-h-[54px] items-center rounded-2xl px-8 text-base font-black shadow-2xl transition hover:scale-[1.03]" style={{ backgroundColor: meta.accent }}>
          ...
          <div className="flex flex-wrap justify-center gap-2 lg:justify-start">
            {['رابط جاهز', 'طلب مباشر', 'جوال أولًا'].map((item) => <span key={item} className="rounded-full bg-white/80 px-3 py-1.5 text-[11px] font-black text-stone-800">✓ {item}</span>)}
          </div>
        </div>

        <div className="flex justify-center lg:justify-start">
          <div className="relative w-[286px] sm:w-[350px]">
            <div className="absolute left-1/2 top-1/2 h-[360px] w-[360px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10 sm:h-[470px] sm:w-[470px]" />
                ...
                <div className="relative mx-3 mt-3 overflow-hidden rounded-2xl p-5 text-white" style={{ background: `linear-gradient(135deg, ${meta.accent}dd, ${meta.accent}88)` }}>
                  ...
                  <div className="mt-6 grid place-items-center">
                    {selectedImage ? (
                      ...
                      <img src={selectedImage} alt={data.title} className="h-28 w-28 rounded-2xl object-cover shadow-2xl ring-[3px] ring-white/30" />
                    ) : (
                      <div className="grid h-28 w-28 place-items-center rounded-2xl bg-white/20 text-5xl">{meta.icon}</div>
                    )}
                  </div>
                  {images.length > 1 ? <div className="mt-3 flex justify-center gap-1.5">{images.map((_, index) => <button key={index} type="button" onClick={() => setActiveImage(index)} className={`h-1.5 rounded-full ${index === activeImage ? 'w-5 bg-white' : 'w-1.5 bg-white/40'}`} aria-label={`الصورة ${index + 1}`} />)}</div> : null}
                </div>
            ...
            <div className="absolute -left-4 top-14 rounded-2xl bg-white/90 px-3 py-2 text-[11px] font-black text-stone-700 shadow-xl">جوال أولًا</div>
            <div className="absolute -right-3 bottom-24 rounded-2xl bg-white/90 px-3 py-2 text-[11px] font-black text-stone-700 shadow-xl">فوري</div>
          </div>
        </div>
      </section>
